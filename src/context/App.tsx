...
import { AuthGate, AppUser } from './components/AuthGate';
...
export default function App() {
  const [user, setUser] = React.useState<AppUser | null>(() => {
    const saved = localStorage.getItem('transitpulse-user');
    return saved ? JSON.parse(saved) : null;
  });
  const login = (nextUser: AppUser) => { setUser(nextUser); localStorage.setItem('transitpulse-user', JSON.stringify(nextUser)); };
  if (!user) return <AuthGate onLogin={login} />;
  return <TransitProvider><MainContentWithRole user={user} onLogout={() => { setUser(null); localStorage.removeItem('transitpulse-user'); }} /></TransitProvider>;
}

const MainContentWithRole: React.FC<{user: AppUser; onLogout: () => void}> = ({ user, onLogout }) => <div><div className="bg-cyan-950 text-cyan-200 px-4 py-2 text-sm flex justify-between items-center"><span>Hi, {user.name} • {user.role}</span><button onClick={onLogout} className="underline">Logout</button></div><MainContent /></div>;
